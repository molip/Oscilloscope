...
void CMainFrame::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == 1)
	{
		...
		if (_serial.IsOpen())
		{
			...
			msg.Format(L"Connection open: Port = %s, Temporal error = %.2f%% Frequency = %.2fHz Min = %.2fV Max = %.2fV Zoom = %dpix/s Grid = %ddiv/s",
				_serial.GetPortName().c_str(), _serial.GetTemporalError() * 100, _frequency, minVal, maxVal, m_wndView.GetZoom(), m_wndView.GetDivsPerSecond());
...
{
	pCmdUI->SetCheck(_serial.IsOpen());
}

void CMainFrame::OnTrigger()
{
	m_wndView.SetTrigger(!m_wndView.IsTrigger());
}
